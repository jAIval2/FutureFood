...
export const Checkout: React.FC = () => {
  ...
  const { cartItems, donationItem, donationEmail, getCartTotal, removeFromCart, updateQuantity, setDonation } = useCart();
                    ...
                    {cartItems.map((item) => (
                      <div key={item.id} className="space-y-2">
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <span className="w-20 text-right text-muted-foreground">${(item.price * item.quantity).toFixed(2)}</span>
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="rounded-xl border border-accent bg-accent/10 p-6">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-muted-foreground">
                        <span>{donationItem.quantity}x {donationItem.name}</span>
                        <span>${(donationItem.price * donationItem.quantity).toFixed(2)}</span>
                      </div>
                      <p className="text-accent-foreground">
                        A photo of your donation will be sent to: {donationEmail}
                      </p>
                    </div>
                  </div>
                )}
